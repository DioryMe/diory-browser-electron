...
### Update Node to match with Electron
...
1. Change that version to `Dockerfile`, e.g. `FROM node:12.16.3`
1. Change that version for all the Github Actions
...
1. Remove the resolutions (see Security fixes for details)
2. ```shell
$ docker-compose run --rm build yarn upgrade --latest
$ git commit package.json yarn.lock -m "Upgrade packages: yarn upgrade --latest"
```
3. Then just check that the tests pass. And that's it!
